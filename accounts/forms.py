    ...
    def __init__(self, *args, **kwargs):
        ...
        for name, field in self.fields.items():
            ...
            if 'id' not in field.widget.attrs:
                field.widget.attrs['id'] = f'id_client-{name}'
        if self.data.get('client-country'):
            from .models import REGION_DATA
            country_code = self.data.get('client-country')
            if country_code in REGION_DATA:
                self.fields['province'].choices = [('', 'Select State/Province')] + REGION_DATA[country_code]
    # Custom fields (already have explicit unique IDs, which is good)
    country = forms.ChoiceField(choices=COUNTRY_CHOICES, required=False, widget=forms.Select(attrs={'id': 'id_client-country', 'class': 'country-select'}))
    province = forms.ChoiceField(choices=[('', 'Select Country First')], required=False, widget=forms.Select(attrs={'id': 'id_client-province', 'class': 'province-select'}))
...
class TherapistRegisterForm(BaseRegisterForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for name, field in self.fields.items():
            # Apply prefix only if the ID hasn't been explicitly set with a unique prefix
            if 'id' not in field.widget.attrs:
                field.widget.attrs['id'] = f'id_therapist-{name}'
        if self.data.get('therapist-country'):
            from .models import REGION_DATA
            country_code = self.data.get('therapist-country')
            if country_code in REGION_DATA:
                self.fields['province'].choices = [('', 'Select State/Province')] + REGION_DATA[country_code]
    ...
    province = forms.ChoiceField(choices=[('', 'Select Country First')], required=True, widget=forms.Select(attrs={'id': 'id_therapist-province', 'class': 'province-select'}))
    street = forms.CharField(required=False, help_text="(Optional)", widget=forms.TextInput(attrs={'id': 'id_therapist-street', 'placeholder': 'Street Address'}))
    phone_number = forms.CharField(required=False, help_text="(Optional)")
